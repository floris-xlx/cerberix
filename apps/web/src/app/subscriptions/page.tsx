...
export default async function SubscriptionsPage() {
  ...
  async function createSub(formData: FormData) {
    ...
    const parsed = z
      .object({ name: z.string().min(1), eventType: z.string().min(1), targetUrl: z.string().url(), secret: z.string().min(8) })
  ...
  const subs = await db.select().from(tables.subscriptions).where(eq(tables.subscriptions.projectId, project.id));
  return (
    <div className="p-8">
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-4">
        <form action={createSub} className="p-4 border rounded-md bg-foreground/5">
          ...
          <input name="eventType" placeholder="Event type" className="w-full mb-2 px-3 py-2 bg-background border rounded-sm" />
          <input name="targetUrl" placeholder="https://example.com/webhook" className="w-full mb-2 px-3 py-2 bg-background border rounded-sm" />
          <input name="secret" placeholder="Signing secret" className="w-full mb-2 px-3 py-2 bg-background border rounded-sm" />
          <div className="flex gap-2">
            <input name="maxRetries" type="number" placeholder="Max retries" className="w-1/2 px-3 py-2 bg-background border rounded-sm" />
            <input name="retryBackoffBaseSeconds" type="number" placeholder="Backoff base s" className="w-1/2 px-3 py-2 bg-background border rounded-sm" />
          </div>
          <button className="mt-3 bg-brand text-white px-4 py-2 rounded-md shadow-none">Create</button>
        </form>
